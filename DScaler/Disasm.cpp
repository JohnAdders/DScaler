    ...
    while(result < result_limit)
    {
        ...
        else if ((unsigned char)c >= 0xe0)
        {
            ...
            case kTarget_ap:
                ...
                break;
            case kTarget_p_cs:
                pContext->pszSegmentOverride = regseg[1];
                break;
            case kTarget_p_ss:
                pContext->pszSegmentOverride = regseg[2];
                break;
            case kTarget_p_ds:
                pContext->pszSegmentOverride = regseg[3];
                break;
            case kTarget_p_es:
                pContext->pszSegmentOverride = regseg[0];
                break;
            case kTarget_p_fs:
                pContext->pszSegmentOverride = regseg[4];
                break;
            case kTarget_p_gs:
                pContext->pszSegmentOverride = regseg[5];
                break;
            case kTarget_p_66:
                pContext->bSizeOverride = true;
                break;
            case kTarget_p_67:
                pContext->bAddressOverride = true;
                break;
            case kTarget_p_F2:
                pContext->bRepnePrefix = true;
                break;
            case kTarget_p_F3:
                pContext->bRepePrefix = true;
                break;
            }
        }
        else
    ...
    while(pattern_len)
    {
        if (!pattern[1] && pattern[0])
        {
            ...
            else
            {
                ...
                {
                case 16:
                    if (!pContext->bSizeOverride)
                    {
                        return NULL;
                    }
                    break;
                case 17:
                    if (!pContext->bAddressOverride)
                    {
                        return NULL;
                    }
                    break;
                case 18:
                    if (!pContext->bRepnePrefix)
                    {
                        return NULL;
                    }
                    break;
                case 19:
                    if (!pContext->bRepePrefix)
                    {
                        return NULL;
                    }
                    break;
...
bool VDDisasmInit(VDDisassemblyContext *pvdc, const char *pszFilename)
{
    ...
    if (memcmp((char *)src + 6, "] DScaler disasm module", 23))
    {
        return false;
    }
