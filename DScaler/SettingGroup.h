...
//	pGroupEx->SetSuspend(SUSPEND_NOTIFY|SUSPEND_JOSTLE, TRUE);
...
protected:
	...
	// settings and put the appropriate ccinfo onto the bulklist.
	virtual void ReviseBulkSettings(IN PBULKCHANGELIST bulklist);
...
protected:
	...
	// Stores if notifications are disabled.
	BOOL						m_silent;
...
public:
	...
	// The current value of every setting is notified with SETTING and SET.
	virtual void ReviseSettings(IN BOOL includeSubgroups = TRUE);
...
protected:
	...
	virtual void ReviseBulkSettings(IN BOOL includeSubgroups, IN PBULKCHANGELIST bulklist);
